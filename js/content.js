...
let timeCheck = null;
// download files data
let downloadButtonsAdded = false;
let moduleItemsCache = {};
let filesDownloadUrlCache = {};
...
async function insertReminders(reminders) {
  ...
  if (toAdd.length > 0 || overrides === true)
    chrome.storage.sync.set({ reminders: [...storage['reminders'], ...toAdd] });
}
...
function createReminder(reminder, location) {
  ...
  const wrapper = makeElement('div', location, {
    className: 'bettercanvas-reminder-wrapper',
  });
  const container = makeElement('div', wrapper, {
    className: 'bettercanvas-reminder-container',
  });
  ...
  const title = makeElement('h2', content, {
    className: 'bettercanvas-reminder-title',
    textContent: reminder.t,
  });
  const due = makeElement('p', content, {
    className: 'bettercanvas-reminder-due',
    textContent: `Assignment due in ${remaining.time}`,
  });
  const hidebtn = makeElement('btn', wrapper, {
    className: 'bettercanvas-reminder-hide',
    textContent: 'x',
  });
...
async function reminderWatch() {
  ...
  if (sync['remind'] !== true) {
    if (document.getElementById('bettercanvas-reminders'))
      document.getElementById('bettercanvas-reminders').style.display = 'none';
    return;
  }
  const container =
    document.getElementById('bettercanvas-reminders') ||
    makeElement('div', document.body, { id: 'bettercanvas-reminders' });
  ...
  storage['reminders'].forEach((reminder, index) => {
    if (reminder.d < now) {
      ...
      (reminder.c == 0 && reminder.d < now + alertPeriod) ||
      (reminder.c == 1 && reminder.d < now + alertPeriod2)
    ) {
...
function showExampleReminder() {
  ...
  const example = createReminder(
    { d: new Date(), t: 'This is an example reminder' },
    location,
  );
  example.querySelector('.bettercanvas-reminder-due').textContent =
    'This notification will pop up in other pages to remind you of incomplete assignments that are due in less than 6 hours.' /*It will notify again at 2 hours if the 'Remind 2x' option is on."*/;
}
...
function isDomainCanvasPage() {
  chrome.storage.sync.get(
    ['custom_domain', 'dark_mode', 'dark_preset', 'device_dark', 'remind'],
    (result) => {
      options = result;
  ...
  Object.keys(changes).forEach((key) => {
    ...
    switch (key) {
      ...
      case 'num_assignments':
        ...
        if (
          document.querySelectorAll('.bettercanvas-card-assignment').length ===
          0
        )
...
function inspectDarkMode(withOutput = false) {
  ...
  document.querySelectorAll('*').forEach((el) => {
    ...
    if (bgcolor) {
      ...
      if (
        ...
        g > 245 &&
        b > 245 &&
        !(r === bg0.r && g === bg0.g && b === bg0.b) &&
        !(r === lnk.r && g === lnk.g && b === lnk.b)
      ) {
        el.style.cssText =
          ';background:' +
          options.dark_preset['background-0'] +
          '!important;color' +
          options.dark_preset['text-0'] +
          '!important;' +
          el.style.cssText;
        if (withOutput === true)
        ...
        b > 225 &&
        b < 245 &&
        !(r === bg1.r && g === bg1.g && b === bg1.b) &&
        !(r === lnk.r && g === lnk.g && b === lnk.b)
      ) {
        el.style.cssText =
          ';background:' +
          options.dark_preset['background-1'] +
          '!important;color' +
          options.dark_preset['text-0'] +
          '!important;' +
          el.style.cssText;
    ...
    if (bordercolor) {
      ...
      if (
        ...
        g > 195 &&
        b > 195 &&
        !(r === bdr.r && g === bdr.g && b === bdr.b) &&
        !(r === lnk.r && g === lnk.g && b === lnk.b)
      ) {
        el.style.cssText =
          'border-color:' +
          options.dark_preset['borders'] +
          '!important;' +
          el.style.cssText;
  ...
  console.log(
    'done fixing dark mode - time:',
    performance.now() - time,
    'total backgrounds changed: ',
    bgcount,
    ', total colors changed: ',
    textcount,
  );
  return {
    selectors: output === '' ? 'no gaps determined' : output,
...
function getCardColors() {
  ...
  let colors = [];
  cards.forEach((card) => {
    let rgbColor = card.querySelector('.ic-DashboardCard__header_hero').style
      .backgroundColor;
    colors.push({
      href: card.querySelector('.ic-DashboardCard__link').href,
...
async function getCards(api = null) {
  let dashboard_cards = api
    ? api
    : await getData(
        `${domain}/api/v1/courses?${
  ...
  chrome.storage.sync.get(
    ...
    (storage) => {
      ...
      if (options['card_method_date'] === true) {
        dashboard_cards.sort((a, b) =>
          b?.created_at > a?.created_at ? 1 : -1,
        );
      } else {
        dashboard_cards.sort(
          (a, b) => (b?.enrollment_term_id || 0) - (a?.enrollment_term_id || 0),
        );
      }
      try {
        dashboard_cards.forEach((card) => {
          if (!card.course_code || count >= (options['card_limit'] || 25))
            return;
          let id = card.id;
          if (!cards || !cards[id]) {
            newCards = true;
        ...
        Object.keys(cards).forEach((key) => {
          ...
          if (found === false) {
            console.log(
              'Deleting ' + key + ' from custom_cards...',
              cards[key],
            );
            cards[key] && delete cards[key];
            cards_2[key] && delete cards_2[key];
...
function setAssignmentState(id, updates) {
  ...
  if (length > 7400) {
    let keys = Object.keys(states).sort(
      (a, b) => states[b].expire - states[a].expire,
    );
    keys.splice(-5);
    let newStates = {};
    keys.forEach((key) => {
...
function createTodoCreateBtn(location) {
  ...
  confirmButton.addEventListener('click', () => {
    chrome.storage.sync.get('custom_assignments_overflow', (overflow) => {
      chrome.storage.sync.get(
        overflow['custom_assignments_overflow'],
        (storage) => {
          let course_id = parseInt(
            location.querySelector('#bettercanvas-custom-course').value,
          );
          ...
          const assignment = {
            ...
            plannable: {
              title: location.querySelector('#bettercanvas-custom-name').value,
            },
            plannable_date:
              location.querySelector('#bettercanvas-custom-date').value +
              'T' +
              location.querySelector('#bettercanvas-custom-time').value +
              ':00',
          ...
          let findOpenOverflow = (num) => {
            ...
            chrome.storage.sync.set(
              { [current_overflow]: storage[current_overflow] },
              () => {
                /* assuming any error is because the limit is exceeded */
                if (chrome.runtime.lastError) {
                  if (
                    num ===
                    overflow['custom_assignments_overflow'].length - 1
                  ) {
                    console.log(
                      'all overflows are full! creating new overflow ' +
                        (overflow['custom_assignments_overflow'].length + 1),
                    );
                    let new_overflow =
                      'custom_assignments_' +
...
function createTodoHeader(location) {
  ...
  if (!options.custom_cards || Object.keys(options.custom_cards).length === 0)
    return;
  let addFillout = makeElement('div', location, {
  ...
  addFillout.innerHTML =
    '<input type="text" placeholder="Name" id="bettercanvas-custom-name" class="bettercanvas-custom-input"></input><select id="bettercanvas-custom-course" class="bettercanvas-custom-input"><option value="" disabled selected>Select course</option></select><div style="display: flex;gap:5px"><input type="date" id="bettercanvas-custom-date"  class="bettercanvas-custom-input"></input><input type="time" id="bettercanvas-custom-time"  class="bettercanvas-custom-input" value="23:59"></input></div>';
  addFillout.querySelector('#bettercanvas-custom-date').value =
    year + '-' + month + '-' + day;
  ...
  let headerText = makeElement('span', todoHeader, {
    className: 'bettercanvas-todo-header',
    textContent: 'To Do',
  });
  let addButton = makeElement('button', todoHeader, {
    className: 'bettercanvas-custom-btn',
    textContent: '+ Add',
  });
...
function createTodoSections(location) {
  ...
  let todoAssignments = makeElement('ul', location, {
    id: 'bettercanvas-todo-list',
  });
  /*
  ...
  let todoAnnouncements = makeElement('ul', location, {
    id: 'bettercanvas-announcement-list',
  });
  /*
    ...
    */
  let loader =
    '<div class="bettercanvas-todo-item-loader"><div style="width: 100px" class="bettercanvas-skeleton-text"></div><div style="width: 200px" class="bettercanvas-skeleton-text"></div><div class="bettercanvas-skeleton-text"></div></div>';
...
function loadBetterTodo() {
  ...
  try {
    ...
    const x_svg =
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M18 6l-12 12"></path><path d="M6 6l12 12"></path></svg>';
    const check_svg =
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M5 12l5 5l10 -10"></path></svg>';
    const tag_svg =
    ...
    const maxAssignmentCount =
      parseInt(options.num_todo_items) + moreAssignmentCount;
    const maxAnnouncementCount =
      parseInt(options.num_todo_items) + moreAnnouncementCount;
    const hr24 = options.todo_hr24;
    const now = new Date();
    //const csrfToken = CSRFtoken();
    let todoAnnouncements = document.querySelector(
      '#bettercanvas-announcement-list',
    );
    ...
    assignments.then((data) => {
      chrome.storage.sync.get(
        options.custom_assignments_overflow,
        (storage) => {
          ...
          items.forEach((item, index) => {
            ...
            if (item.plannable_type !== 'announcement') {
              ...
              if (
                filter === 'todo' &&
                options.hide_completed === true &&
                item.submissions.submitted === true
              )
                return;
              if (
                filter === 'todo' &&
                ((options.todo_overdues !== true && now >= date) ||
                  (options.todo_overdues === true &&
                    item.submissions.submitted === true))
              )
                return;
              if (
            ...
            if (options.todo_colors === true)
              title.style =
                'color:' +
                (options.custom_cards_3?.[item.course_id]?.color || 'inherit') +
                '!important;';
            ...
            if (options.hover_preview === true) {
              const customItem =
                item.planner_override &&
                item.planner_override.custom &&
                item.planner_override.custom === true;
              listItem.addEventListener('mouseover', () => {
                listItem.classList.add('bettercanvas-todo-hover');
                let preview = listItemContainer.querySelector(
                  '.bettercanvas-hover-preview',
                );
                let previewTitle = preview.querySelector(
                  '.bettercanvas-preview-title',
                );
                let previewText = preview.querySelector(
                  '.bettercanvas-preview-text',
                );
                clearTimeout(delay);
            ...
            const actions = listItemContainer.querySelector(
            ...
            listItemContainer
              .querySelector('.bettercanvas-todo-actions-btn')
            ...
            crossOffBtn.addEventListener('click', () => {
              setAssignmentState(item.plannable_id, {
                crs:
                  listItemContainer.querySelector('.bettercanvas-todo-item')
                    .style.textDecoration === 'line-through'
                    ? false
                    : true,
                expire: dueAt,
                ...
                chrome.storage.sync.get(
                  ...
                  (overflow) => {
                    chrome.storage.sync.get(
                      overflow['custom_assignments_overflow'],
                      (storage) => {
                        overflow['custom_assignments_overflow'].forEach(
                          (overflow) => {
                            for (let i = 0; i < storage[overflow].length; i++) {
                              if (
                                storage[overflow][i].plannable_id ===
                                item.plannable_id
                              ) {
                                storage[overflow].splice(i, 1);
                                chrome.storage.sync
                                  .set({ [overflow]: storage[overflow] })
                                  .then(() => {});
                                break;
                              }
                            }
...
*/
            ...
            if (item.plannable_type === 'announcement') {
              ...
              if (item.submissions && item.submissions.submitted) {
                listItemContainer.classList.add(
                  'bettercanvas-todo-item-completed',
                );
              }
            }
            //}
          ...
          if (assignmentsToInsert.length > 0) {
            ...
            for (
              i = 0;
              i <
              (assignmentsToInsert.length > maxAssignmentCount
                ? maxAssignmentCount
            ...
            if (i !== assignmentsToInsert.length)
              createTodoViewMore(todoAssignments, 'assignment');
          } else {
            makeElement('p', todoAssignments, {
              className: 'bettercanvas-none-due',
              textContent: 'None',
            });
          }
          ...
          if (announcementsToInsert.length > 0) {
            let i;
            for (
              i = announcementsToInsert.length - 1;
              i >=
              (announcementsToInsert.length - maxAnnouncementCount < 0
                ? 0
                : announcementsToInsert.length - maxAnnouncementCount);
              i--
            ) {
            ...
            makeElement('p', todoAnnouncements, {
              className: 'bettercanvas-none-due',
              textContent: 'None',
            });
          }
...
async function changeColorPreset(colors) {
  ...
  try {
    sortedCards.forEach((card, i) => {
      let previousColor = rgbToHex(
        card.el.querySelector('.ic-DashboardCard__header_hero').style
          .backgroundColor,
      );
  ...
  chrome.storage.local.get('previous_colors', (local) => {
    const now = Date.now();
    if (
      local['previous_colors'] === null ||
      now >= local['previous_colors'].expire
    ) {
      chrome.storage.local.set({
        previous_colors: { colors: previous, expire: now + 86400000 },
      });
    }
  });
}
...
function toggleDarkMode() {
  ...
  if (
    (options.dark_mode === true || options.device_dark === true) &&
    !darkStyleInserted
  ) {
    ...
    style.textContent =
      options.dark_mode === true || options.device_dark ? css : '';
    style.className =
      options.dark_mode === true || options.device_dark
        ? 'bettercanvas-darkmode-enabled'
        : '';
  }
  /*
...
function runDarkModeFixer(override = false) {
  if (options.dark_mode !== true)
    return { path: 'bettercanvas-darkmode_off', time: '' };
  if (
    override === false &&
    !options['dark_mode_fix'].includes(window.location.pathname)
  )
    return { path: 'bettercanvas-none', time: '' };
...
function runiframeChecker() {
  ...
  if (options.dark_mode !== true) {
    ...
    document.querySelectorAll('iframe').forEach((frame) => {
      if (
        ...
        frame.contentDocument.documentElement.querySelector('#darkcss')
      ) {
        frame.contentDocument.documentElement.querySelector(
          '#darkcss',
        ).textContent = '';
        frame.contentDocument.body.classList.remove(
          'bettercanvas--darkmode--enabled',
        );
      }
    });
  ...
  iframeObserver.observe(document.querySelector('html'), {
    childList: true,
    subtree: true,
  });
}
  ...
  if (options.dashboard_grades === true) {
    grades.then((data) => {
      try {
        ...
        if (
          cards.length === 0 ||
          cards[0].querySelectorAll('.ic-DashboardCard__link').length === 0
        )
          return;
        for (let i = 0; i < cards.length; i++) {
          ...
          data.forEach((grade) => {
            if (course_id === grade.id) {
              let gradepercent =
                grade.enrollments[0].has_grading_periods === true
                  ? grade.enrollments[0].current_period_computed_current_score
                  : grade.enrollments[0].computed_current_score;
              //let gradepercent = grade.enrollments[0].computed_current_score;
              let percent = (gradepercent || '--') + '%';
              let gradeContainer =
                cards[i].querySelector('.bettercanvas-card-grade') ||
                makeElement(
                  'a',
                  cards[i].querySelector('.ic-DashboardCard__header'),
                  {
              ...
              gradeContainer.setAttribute(
                'href',
                `${domain}/courses/${course_id}/grades`,
              );
...
function createCardAssignment(assignment) {
  ...
  if (assignment.overdue === true)
    assignmentDueAt.classList.add('bettercanvas-assignment-overdue');
  ...
  assignmentDueAt.addEventListener('mouseup', function () {
    ...
    const status = assignmentContainer.classList.contains(
      'bettercanvas-completed',
    );
    setAssignmentState(assignment.plannable_id, {
      crs: status,
      expire: assignment.plannable_date,
    });
  });
    ...
    assignments.then((data) => {
      ...
      data.forEach((item) => {
        ...
        let o = {
          ...
          override:
            item.planner_override && item.planner_override.marked_complete,
...
function loadCardAssignments() {
  if (options.assignments_due !== true) {
    document
      .querySelectorAll('.bettercanvas-card-assignment')
      .forEach((card) => {
      ...
      cards.forEach((card) => {
        ...
        if (els[course_id]) {
          els[course_id].forEach((assignment) => {
            ...
            if (
              options.hide_completed_cards === true &&
              assignment.submitted === true
            )
              return;
            if (
              (options.card_overdues !== true && now >= assignment.due) ||
              (options.card_overdues === true && assignment.submitted === true)
            )
              return;
            if (
              assignment.type !== 'assignment' &&
              assignment.type !== 'quiz' &&
              assignment.type !== 'discussion_topic'
            )
              return;
        ...
        if (count === 0) {
          let assignmentContainer = makeElement('div', cardContainer, {
            className: 'bettercanvas-assignment-container',
          });
...
function setupCardAssignments() {
  ...
  try {
    if (
      ...
      document.querySelectorAll('.bettercanvas-card-container').length > 0
    )
      return;
    let cards = document.querySelectorAll('.ic-DashboardCard');
    cards.forEach((card) => {
      let assignmentContainer =
        card.querySelector('.bettercanvas-card-assignment') ||
        makeElement('div', card, { className: 'bettercanvas-card-assignment' });
      let assignmentsDueHeader =
        card.querySelector('.bettercanvas-card-header-container') ||
        makeElement('div', assignmentContainer, {
          className: 'bettercanvas-card-header-container',
        });
      let assignmentsDueLabel =
        card.querySelector('.bettercanvas-card-header') ||
        makeElement('h3', assignmentsDueHeader, {
          className: 'bettercanvas-card-header',
          textContent: chrome.i18n.getMessage('due'),
        });
      let cardContainer =
        card.querySelector('.bettercanvas-card-container') ||
        makeElement('div', assignmentContainer, {
          className: 'bettercanvas-card-container',
        });
      let skeletonText =
        card.querySelector('.bettercanvas-skeleton-text') ||
        makeElement('div', cardContainer, {
          className: 'bettercanvas-skeleton-text',
        });
    });
  } catch (e) {
    logError(e);
...
function customizeCards(c = null) {
  ...
  try {
    ...
    cards.forEach((card) => {
      ...
      // hide card
      card.style.display =
        cardOptions.hidden === true ? 'none' : 'inline-block';
      ...
      if (cardOptions.img === 'none') {
        ...
        let container =
          card.querySelector('.ic-DashboardCard__header_image') ||
          makeElement('div', card, {
      ...
      if (cardOptions.name !== '') {
        card.querySelector(
          '.ic-DashboardCard__header-title > span',
        ).textContent = cardOptions.name;
      }

      // card code
      if (cardOptions.code !== '') {
        card.querySelector('.ic-DashboardCard__header-subtitle').textContent =
          cardOptions.code;
      }
      ...
      for (let i = links.length; i < 4; i++) {
        makeElement(
          'a',
          card.querySelector('.ic-DashboardCard__action-container'),
      ...
      for (let i = 0; i < 4; i++) {
        let img =
          links[i].querySelector('.bettercanvas-link-image') ||
          makeElement('img', links[i], {
            className: 'bettercanvas-link-image',
          });
        ...
        if (cardOptions_2.links[i].path === 'none') {
          ...
          if (links[i].querySelector('.ic-DashboardCard__action-layout'))
            links[i].querySelector(
...
function calculateGPA2() {
  ...
  document.querySelectorAll('.bettercanvas-gpa-course').forEach((course) => {
    ...
    const credits = parseFloat(
      course.querySelector('.bettercanvas-course-credit').value,
    );
    const grade = parseFloat(
      course.querySelector('.bettercanvas-course-percent').value,
    );
  ...
  document.querySelector('#bettercanvas-gpa-unweighted').textContent = (
    qualityPoints / numCredits
  ).toFixed(2);
  document.querySelector('#bettercanvas-gpa-weighted').textContent = (
    weightedQualityPoints / numCredits
  ).toFixed(2);
  const cGPA = document.querySelector('#bettercanvas-cumulative-gpa');
  const g = parseFloat(
    cGPA.querySelector('.bettercanvas-course-percent').value,
  );
  const c = parseInt(cGPA.querySelector('.bettercanvas-course-credit').value);
  document.querySelector('#bettercanvas-gpa-cumulative').textContent = (
    ((options.gpa_calc_weighted === true
...
function changeGPASettings(course_id, update) {
  ...
  chrome.storage.sync.get(['custom_cards', 'cumulative_gpa'], (storage) => {
    if (course_id === 'cumulative') {
      chrome.storage.sync.set({
        cumulative_gpa: { ...storage['cumulative_gpa'], ...update },
      });
    } else {
      chrome.storage.sync.set({
        custom_cards: {
...
function createGPACalcCourse(location, course) {
  ...
  let courseContainer = makeElement('div', location, {
    className:
      course.id === 'cumulative'
        ? 'bettercanvas-gpa-cumulative'
  ...
  let changerContainer = makeElement('div', courseContainer, {
    className: 'bettercanvas-gpa-percent-container',
  });

  let credits = makeElement('div', courseContainer, {
    className: 'bettercanvas-course-credits',
    innerHTML:
      '<input class="bettercanvas-course-credit" value="1"></input><span class="bettercanvas-course-percent-sign">cr</span>',
  });
  let creditsChanger = credits.querySelector('.bettercanvas-course-credit');
  creditsChanger.value = customs.credits;
  let changer = makeElement('input', changerContainer, {
    className: 'bettercanvas-course-percent',
  });
  ...
  let courseGrade =
    course?.enrollments[0].has_grading_periods === true
  ...
  if (course.id !== 'cumulative') {
    let weightSelections = makeElement('form', courseContainer, {
      className: 'bettercanvas-course-weights',
    });
    weightSelections.innerHTML =
      '<select name="weight-selection" class="bettercanvas-course-weight"><option value="dnc">Do not count</option><option value="regular">Regular/College</option><option value="honors">Honors</option><option value="ap">AP/IB</option></select>';
    let weightChanger = weightSelections.querySelector(
      '.bettercanvas-course-weight',
    );
    weightChanger.value = changer.value === '--' ? 'dnc' : customs.weight;
    weightChanger.addEventListener('change', () =>
      changeGPASettings(course.id, {
        weight: weightSelections.querySelector('.bettercanvas-course-weight')
          .value,
      }),
    );
    ...
    useCustomGr.addEventListener('input', () => {
      if (options['custom_cards'][course.id]) {
        if (
          options['custom_cards'][course.id]['gr'] !== undefined &&
          options['custom_cards'][course.id]['gr'] !== null
        ) {
          changer.value = courseGrade;
  ...
  credits
    .querySelector('.bettercanvas-course-credit')
    .addEventListener('input', () =>
      changeGPASettings(course.id, {
        credits: credits.querySelector('.bettercanvas-course-credit').value,
      }),
    );
...
function setupGPACalc() {
  ...
  try {
    grades?.then((result) => {
      ...
      let container2 =
        document.querySelector('.bettercanvas-gpa-card') ||
        document.createElement('div');
      container2.className = 'bettercanvas-gpa-card';
      container2.style.display =
        options.gpa_calc === true ? 'inline-block' : 'none';

      container2.innerHTML = `<h3 class="bettercanvas-gpa-header">GPA</h3><div><div><p id="bettercanvas-gpa-unweighted"></p><p>Current</p></div><div style="display:${
        options['gpa_calc_weighted'] ? 'block' : 'none'
      }"><p id="bettercanvas-gpa-weighted"></p><p>Weighted</p></div><div style="display:${
        options['gpa_calc_cumulative'] ? 'block' : 'none'
      }"><p id="bettercanvas-gpa-cumulative"></p><p>Cumulative</p></div></div>`;
      ...
      let container =
        document.querySelector('.bettercanvas-gpa') ||
        document.createElement('div');
      container.className = 'bettercanvas-gpa';
      container.innerHTML =
        '<h3 class="bettercanvas-gpa-header">GPA Calculator</h3><div class="bettercanvas-gpa-courses-container"><div class="bettercanvas-gpa-courses"></div></div>';

      if (options.gpa_calc_prepend === true) {
        document
          .querySelector('.ic-DashboardCard__box__container')
          .prepend(container2);
        document
          .querySelector('.ic-DashboardCard__box__container')
          .prepend(container);
...
function loadDashboardNotes() {
  if (options.dashboard_notes === true) {
    let notes =
      document.querySelector('.bettercanvas-dashboard-notes') ||
...
function loadCustomFont() {
  ...
  let load = () => {
    ...
    style.textContent =
      options.custom_font.link === ''
        ? ''
        : `*, input, a, button, h1, h2, h3, h4, h5, h6, p, span {font-family: ${options.custom_font.family}!important}`;
  };
...
function applyAestheticChanges() {
  let style =
    document.querySelector('#bettercanvas-aesthetics') ||
    document.createElement('style');
  style.id = 'bettercanvas-aesthetics';
  style.textContent = '';
  if (options.condensed_cards === true)
    style.textContent +=
      '.ic-DashboardCard__header_hero {height:60px!important}.ic-DashboardCard__header-subtitle, .ic-DashboardCard__header-term{display:none}';
  ...
  if (options.disable_color_overlay === true)
    style.textContent +=
      '.ic-DashboardCard__header_hero{opacity: 0!important} .ic-DashboardCard__header-button-bg{opacity: 1!important}';
  if (options.hide_feedback === true)
...
function changeGradientCards() {
  if (options.gradient_cards === true) {
    ...
    let cardcss =
      document.querySelector('#gradientcss') || document.createElement('style');
    ...
    for (let i = 0; i < cardheads.length; i++) {
      ...
      let degree =
        (h % 60) / 60 >= 0.66 ? 30 : (h % 60) / 60 <= 0.33 ? -30 : 15;
      let newh = h > 300 ? 360 - (h + 65) + (65 + degree) : h + 65 + degree;
      cardcss.textContent +=
        '.ic-DashboardCard:nth-of-type(' +
        (i + 1) +
        ') .ic-DashboardCard__header_hero{background: linear-gradient(115deg, hsl(' +
        h +
        'deg,' +
        s +
        '%,' +
...
function showUpdateMsg() {
  ...
  const close = makeElement('button', div, {
    id: 'bettercanvas-update-close',
    textContent: 'Close',
  });
...
function combineAssignments(data) {
  ...
  return combined.sort(
    (a, b) =>
      new Date(a.plannable_date).getTime() -
      new Date(b.plannable_date).getTime(),
  );
}
...
function cleanCustomAssignments() {
  chrome.storage.sync.get('custom_assignments_overflow', (overflows) => {
    chrome.storage.sync.get(
      overflows['custom_assignments_overflow'],
      (storage) => {
        const now = new Date();
        ...
        overflows['custom_assignments_overflow'].forEach((overflow) => {
          ...
          if (changed)
            chrome.storage.sync.set({ [overflow]: storage[overflow] });
        });
      },
    );
  });
...
function setupCustomURL() {
  ...
  let test = getData(
    `${domain}/api/v1/courses?${/*enrollment_state=active&*/ ''}per_page=100`,
  );
  test
    .then((res) => {
      if (res.length) {
        ...
        console.log(
          "Better Canvas - this url doesn't seem to be a canvas url (1)",
        );
      }
    })
    .catch((err) => {
      console.log(
        "Better Canvas - this url doesn't seem to be a canvas url (2)",
      );
    });
}
...
function getGrades() {
  if (options.gpa_calc === true || options.dashboard_grades === true) {
    grades = getData(
      `${domain}/api/v1/courses?${
        /*enrollment_state=active&*/ ''
      }include[]=concluded&include[]=total_scores&include[]=computed_current_score&include[]=current_grading_period_scores&per_page=100`,
    );
  }
}
  ...
  if (options.tab_icons || options.todo_colors) {
    ...
    colors.then((data) => {
      ...
      Object.keys(cards).forEach((key) => {
        cards[key] = {
          ...cards[key],
          color: data['custom_colors']['course_' + key]
            ? data['custom_colors']['course_' + key]
            : null,
        };
      });
...
function changeFavicon() {
  ...
  if (
    match &&
    match.groups.id &&
    options.custom_cards_3[match.groups.id]?.color
  ) {
    document.querySelector(
      'link[rel="icon"',
    ).href = `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" fill="white" width="128px" height="128px" viewBox="-192 -192 2304.00 2304.00" stroke="white"><g stroke-width="0"><rect x="-192" y="-192" width="2304.00" height="2304.00" rx="0" fill="${options.custom_cards_3[
      match.groups.id
    ].color.replace(
      '#',
      '%23',
    )}" strokewidth="0"/></g><g stroke-linecap="round" stroke-linejoin="round"/><g> <path d="M958.568 277.97C1100.42 277.97 1216.48 171.94 1233.67 34.3881 1146.27 12.8955 1054.57 0 958.568 0 864.001 0 770.867 12.8955 683.464 34.3881 700.658 171.94 816.718 277.97 958.568 277.97ZM35.8207 682.031C173.373 699.225 279.403 815.285 279.403 957.136 279.403 1098.99 173.373 1215.05 35.8207 1232.24 12.8953 1144.84 1.43262 1051.7 1.43262 957.136 1.43262 862.569 12.8953 769.434 35.8207 682.031ZM528.713 957.142C528.713 1005.41 489.581 1044.55 441.31 1044.55 393.038 1044.55 353.907 1005.41 353.907 957.142 353.907 908.871 393.038 869.74 441.31 869.74 489.581 869.74 528.713 908.871 528.713 957.142ZM1642.03 957.136C1642.03 1098.99 1748.06 1215.05 1885.61 1232.24 1908.54 1144.84 1920 1051.7 1920 957.136 1920 862.569 1908.54 769.434 1885.61 682.031 1748.06 699.225 1642.03 815.285 1642.03 957.136ZM1567.51 957.142C1567.51 1005.41 1528.38 1044.55 1480.11 1044.55 1431.84 1044.55 1392.71 1005.41 1392.71 957.142 1392.71 908.871 1431.84 869.74 1480.11 869.74 1528.38 869.74 1567.51 908.871 1567.51 957.142ZM958.568 1640.6C816.718 1640.6 700.658 1746.63 683.464 1884.18 770.867 1907.11 864.001 1918.57 958.568 1918.57 1053.14 1918.57 1146.27 1907.11 1233.67 1884.18 1216.48 1746.63 1100.42 1640.6 958.568 1640.6ZM1045.98 1480.11C1045.98 1528.38 1006.85 1567.51 958.575 1567.51 910.304 1567.51 871.172 1528.38 871.172 1480.11 871.172 1431.84 910.304 1392.71 958.575 1392.71 1006.85 1392.71 1045.98 1431.84 1045.98 1480.11ZM1045.98 439.877C1045.98 488.148 1006.85 527.28 958.575 527.28 910.304 527.28 871.172 488.148 871.172 439.877 871.172 391.606 910.304 352.474 958.575 352.474 1006.85 352.474 1045.98 391.606 1045.98 439.877ZM1441.44 1439.99C1341.15 1540.29 1333.98 1697.91 1418.52 1806.8 1579 1712.23 1713.68 1577.55 1806.82 1418.5 1699.35 1332.53 1541.74 1339.7 1441.44 1439.99ZM1414.21 1325.37C1414.21 1373.64 1375.08 1412.77 1326.8 1412.77 1278.53 1412.77 1239.4 1373.64 1239.4 1325.37 1239.4 1277.1 1278.53 1237.97 1326.8 1237.97 1375.08 1237.97 1414.21 1277.1 1414.21 1325.37ZM478.577 477.145C578.875 376.846 586.039 219.234 501.502 110.339 341.024 204.906 206.338 339.592 113.203 498.637 220.666 584.607 378.278 576.01 478.577 477.145ZM679.155 590.32C679.155 638.591 640.024 677.723 591.752 677.723 543.481 677.723 504.349 638.591 504.349 590.32 504.349 542.048 543.481 502.917 591.752 502.917 640.024 502.917 679.155 542.048 679.155 590.32ZM1440 475.712C1540.3 576.01 1697.91 583.174 1806.8 498.637 1712.24 338.159 1577.55 203.473 1418.51 110.339 1332.54 217.801 1341.13 375.413 1440 475.712ZM1414.21 590.32C1414.21 638.591 1375.08 677.723 1326.8 677.723 1278.53 677.723 1239.4 638.591 1239.4 590.32 1239.4 542.048 1278.53 502.917 1326.8 502.917 1375.08 502.917 1414.21 542.048 1414.21 590.32ZM477.145 1438.58C376.846 1338.28 219.234 1331.12 110.339 1415.65 204.906 1576.13 339.593 1710.82 498.637 1805.39 584.607 1696.49 577.443 1538.88 477.145 1438.58ZM679.155 1325.37C679.155 1373.64 640.024 1412.77 591.752 1412.77 543.481 1412.77 504.349 1373.64 504.349 1325.37 504.349 1277.1 543.481 1237.97 591.752 1237.97 640.024 1237.97 679.155 1277.1 679.155 1325.37Z"/></g></svg>`;
  }
}
...
function rgbToHex(rgb) {
  try {
    ...
    return (
      '#' +
      parseInt(exec[1]).toString(16).padStart(2, '0') +
      parseInt(exec[2]).toString(16).padStart(2, '0') +
      parseInt(exec[3]).toString(16).padStart(2, '0')
    );
  } catch (e) {
    console.warn(e);
  }
...
function formatTodoDate(date, submissions, hr24) {
  ...
  return {
    ...
    date:
      ...
      date.getDate() +
      ' at ' +
      (date.getHours() - (hr24 ? '' : date.getHours() > 12 ? 12 : 0)) +
      ':' +
      (date.getMinutes() < 10 ? '0' : '') +
      date.getMinutes() +
      (hr24 ? '' : date.getHours() >= 12 ? 'pm' : 'am') +
      ' (' +
      fromNow +
      ')',
  };
...
function formatCardDue(date) {
  ...
  return options.assignment_date_format
...
/**
 * Add simple snackbar to the top right of the page
 */
function snack(message) {
  let snack;
  if (document.querySelector('.bettercanvas-snack')) {
    snack = document.querySelector('.bettercanvas-snack');
  } else {
    snack = document.createElement('div');
    snack.classList.add('bettercanvas-snack');
    snack.style = `
      position: fixed;
      top: 10px;
      right: 10px;
...
/**
 * Add button for each Module accordion to download all module's slides
 */
function checkModuleListReady() {
  if (!current_page?.includes('modules')) return;
  if (downloadButtonsAdded) return;
  const callback = (mutationList) => {
    for (const mutation of mutationList) {
      if (mutation.type === 'childList') {
        if (mutation.target == document.querySelector('#context_modules')) {
          addDownloadSlidesButtons();
        }
      }
    }
  };
...
function addDownloadSlidesButtons() {
  if (downloadButtonsAdded) return;
  ...
  modules.forEach((module) => {
    ...
    const { moduleId, moduleUrl } = module.dataset;
    const button = document.createElement('button');
    button.textContent = 'Download Slides';
    button.style = `
      margin-left: 10px;
    ...
    button.addEventListener('click', () => {
      downloadSlides(moduleUrl);
    });
    const moduleHeader = module.querySelector('.ig-header');
    moduleHeader.appendChild(button);
  });
  downloadButtonsAdded = true;
}

// get data from /api/v1/courses/<courseId>/modules/<moduleId>/items
async function downloadSlides(moduleUrl) {
  // moduleUrl = '/courses/123/modules/456'
  let moduleItems;
  if (moduleItemsCache[moduleUrl]) {
    moduleItems = moduleItemsCache[moduleUrl];
  } else {
    moduleItems = await getData(
      `${domain}/api/v1${moduleUrl}/items?per_page=50`,
    );
  }

  const slides = moduleItems.filter((item) => item.type === 'File');
  const fileDownloadUrls = [];
  for (let slide of slides) {
    const { url } = slide;
    if (filesDownloadUrlCache[url]) {
      fileDownloadUrls.push(filesDownloadUrlCache[url]);
    } else {
      const fileData = await getData(url);
      fileDownloadUrls.push(fileData.url);
      filesDownloadUrlCache[url] = fileData.url;
    }
  }

  if (!fileDownloadUrls?.length) return;
